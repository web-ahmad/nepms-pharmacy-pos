...
def create_audit_session(db: Session, data: AuditSessionCreate, tenant_id: str, branch_id: str, user_id: str) -> AuditSession:
    ...
    if data.scope_type == "Category":
        query = query.filter(Medicine.category_id == data.scope_value)
    elif data.scope_type == "Location":
        query = query.filter(Medicine.shelf == data.scope_value)
        ...
        if batch and batch.status == "Active" and not getattr(batch, 'is_deleted', False):
            ...
            if key not in processed:
                item = AuditItem(
                    ...
                    physical_count=None,
                    unit_price=batch.purchase_price or med.purchase_price or 0.0
                )
                db.add(item)
                processed.add(key)
        elif not batch:
            ...
            if key not in processed:
                item = AuditItem(
                    ...
                    system_quantity=0,
                    physical_count=None,
                    unit_price=med.purchase_price or 0.0
                )
                db.add(item)
                processed.add(key)
...
def update_physical_count(db: Session, session_id: str, item_id: str, physical_count: int, tenant_id: str):
