...
export default function ReceivablesBookPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <h2 className="text-xl font-semibold text-zinc-900 dark:text-zinc-50">Receivables Book</h2>
      </div>
            ...
            <div className="flex items-center justify-between border-b border-gray-100 px-4 py-3 dark:border-zinc-800">
              ...
              <DataExportMenu 
                ...
                columns={[
                  ...
                  { header: 'Invoice/Ref ID', accessorKey: 'reference' },
                  { header: 'Customer Name', accessorKey: (row: any) => row.line_desc || row.journal_desc },
                  { header: 'Debit (Amount Given)', accessorKey: 'debit' },
                  { header: 'Credit (Amount Paid)', accessorKey: 'credit' },
              ...
              <table className="w-full text-left text-sm whitespace-nowrap">
                <thead className="sticky top-0 bg-gray-50 dark:bg-zinc-900 z-10 outline outline-1 outline-gray-200 dark:outline-zinc-800">
                  <tr>
                    ...
                    <th className="px-4 py-3 font-medium text-gray-500 dark:text-zinc-400">Invoice/Ref ID</th>
                    <th className="px-4 py-3 font-medium text-gray-500 dark:text-zinc-400">Customer Name / Description</th>
                    <th className="px-4 py-3 font-medium text-gray-500 dark:text-zinc-400 text-right">Debit (Given)</th>
                    <th className="px-4 py-3 font-medium text-gray-500 dark:text-zinc-400 text-right">Credit (Paid)</th>
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-zinc-800">
                  {filteredRows.length === 0 ? (
                    ...
                    filteredRows.map((row: any, idx: number) => {
                      const link = getReferenceLink(row.reference, { source_module: row.source_module, source_id: row.source_id });
                      return (
